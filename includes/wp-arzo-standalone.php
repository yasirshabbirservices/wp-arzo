...
<body>
    <div class="container">
        ...
        <div class="nav">
            <a href="<?php echo admin_url('admin-ajax.php?action=wp_arzo_standalone&tab=info'); ?>" <?php echo ($action === 'info') ? 'class="active"' : ''; ?>>Site Info</a>
            <a href="<?php echo admin_url('admin-ajax.php?action=wp_arzo_standalone&tab=users'); ?>" <?php echo ($action === 'users') ? 'class="active"' : ''; ?>>Users</a>
            <a href="<?php echo admin_url('admin-ajax.php?action=wp_arzo_standalone&tab=database'); ?>" <?php echo ($action === 'database') ? 'class="active"' : ''; ?>>Database</a>
            <a href="<?php echo admin_url('admin-ajax.php?action=wp_arzo_standalone&tab=files'); ?>" <?php echo ($action === 'files') ? 'class="active"' : ''; ?>>Files</a>
            <a href="<?php echo admin_url('admin-ajax.php?action=wp_arzo_standalone&tab=plugins'); ?>" <?php echo ($action === 'plugins') ? 'class="active"' : ''; ?>>Plugins</a>
            <a href="<?php echo admin_url('admin-ajax.php?action=wp_arzo_standalone&tab=themes'); ?>" <?php echo ($action === 'themes') ? 'class="active"' : ''; ?>>Themes</a>
            <a href="<?php echo admin_url('admin-ajax.php?action=wp_arzo_standalone&tab=debug'); ?>" <?php echo ($action === 'debug') ? 'class="active"' : ''; ?>>Debug</a>
            <a href="<?php echo admin_url('admin-ajax.php?action=wp_arzo_standalone&tab=maintenance'); ?>" <?php echo ($action === 'maintenance') ? 'class="active"' : ''; ?>>Maintenance Modes</a>
            <a href="<?php echo admin_url('admin-ajax.php?action=wp_arzo_standalone&tab=extra_options'); ?>" <?php echo ($action === 'extra_options') ? 'class="active"' : ''; ?>>Extra Options</a>
            <a href="<?php echo admin_url('admin-ajax.php?action=wp_arzo_standalone&tab=login'); ?>" <?php echo ($action === 'login') ? 'class="active"' : ''; ?>>Quick Login</a>
        </div>
    ...
    <script>
        ...
        function viewFile(filePath) {
            const baseUrl = '<?php echo admin_url('admin-ajax.php'); ?>';
        ...
        function saveFile(filePath) {
            ...
            fetch('<?php echo admin_url('admin-ajax.php?action=wp_arzo_standalone&tab=ajax&operation=save_file'); ?>', {
                method: 'POST',
                body: formData
            })
                .then(response => response.json())
        ...
        function logDebugChange(settingName, newValue) {
            ...
            fetch('<?php echo admin_url('admin-ajax.php?action=wp_arzo_standalone&tab=ajax&operation=log_debug_change'); ?>', {
                method: 'POST',
                body: formData
            })
                .then(response => response.json())
        ...
        function clearDebugLog() {
            if (confirm('Are you sure you want to clear the debug log?')) {
                ...
                fetch('<?php echo admin_url('admin-ajax.php?action=wp_arzo_standalone&tab=ajax&operation=clear_debug_log'); ?>', {
                    method: 'POST'
                })
                    .then(response => response.json())
                    .then(data => {
            ...
            <form method="post">
                <div class="form-group" style="margin-bottom: 20px;">
                    ...
                    <select name="target_file" class="modern-select" required>
                        <option value="wp-config" <?php echo $wp_config_writable ? '' : 'disabled'; ?>>wp-config.php <?php echo $wp_config_writable ? '' : '(Not writable)'; ?></option>
                        <option value="htaccess" <?php echo $htaccess_writable ? '' : 'disabled'; ?>>.htaccess <?php echo $htaccess_writable ? '' : '(Not writable)'; ?></option>
                        <option value="php-ini" <?php echo $php_ini_writable ? '' : 'disabled'; ?>>php.ini <?php echo $php_ini_writable ? '' : '(Not writable)'; ?></option>
                    </select>
                </div>
                ...
                <div class="php-limits-grid">
                    <div class="form-group">
                        ...
                        <input type="text" name="memory_limit" value="<?php echo $memory_limit; ?>" class="php-limits-input" required>
                    </div>

                    <div class="form-group">
                        <label>Max Execution Time (seconds)</label>
                        <input type="number" name="max_execution_time" value="<?php echo $max_execution_time; ?>" class="php-limits-input" required>
                    </div>

                    <div class="form-group">
                        <label>Upload Max Filesize (e.g. 64M)</label>
                        <input type="text" name="upload_max_filesize" value="<?php echo $upload_max_filesize; ?>" class="php-limits-input" required>
                    </div>

                    <div class="form-group">
                        <label>Post Max Size (e.g. 64M)</label>
                        <input type="text" name="post_max_size" value="<?php echo $post_max_size; ?>" class="php-limits-input" required>
                    </div>
                </div>
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                function loadPluginsPage(page) {
                    const baseUrl = '<?php echo admin_url('admin-ajax.php?action=wp_arzo_standalone'); ?>';
                    fetch(
                        `${baseUrl}&tab=ajax&operation=get_plugins_page&page=${page}&per_page=${perPage}&nocache=${new Date().getTime()}`
                    )
                        .then(response => response.json())
                        .then(data => {
                            if (data.success) {
                    ...
                    <?php foreach ($debug_settings as $setting => $info): ?>
                        <div class="form-group"
                            ...
                            <div style="margin-left: 15px; flex-shrink: 0;">
                                <label class="switch">
                                    <input type="checkbox" name="<?php echo $setting; ?>" value="true" <?php echo ($info['current'] === 'true') ? 'checked' : ''; ?>
        ...
        <?php if ($current_mode): ?>
            <div style="background: #2A2A2A; padding: 20px; border-radius: 3px; border: 1px solid #333333;">
                ...
                <p><strong>Bypass URL:</strong> <a
                        ...
                        target="_blank"
                        style="color: var(--accent-color);"><?php echo home_url('?maintenance_bypass=' . wp_create_nonce('maintenance_bypass')); ?></a>
                </p>
