...
/**
 * WP Arzo — offline QR-code generator (bundled, no external services for privacy).
 *
 * Wraps the well-known MIT-licensed "QRCode for PHP" library by Kazuhiko Arase
 * (http://www.d-project.com/, MIT — the original attribution header is preserved below).
 * Class names are prefixed WP_Arzo_QR* to avoid collisions. Used to render the
 * authenticator `otpauth://` secret as a scannable QR image for Two-Factor enrollment.
 *
 * @package WP_Arzo
 */
...
{
    /**
     * Return a PNG data-URI QR for $text (e.g. an otpauth:// URI), or '' if the GD
     * extension isn't available (callers then fall back to the manual key / HTML table).
     *
     * @param string $text
     * @param int    $module_size Pixels per QR module.
     * @param int    $margin      Quiet-zone pixels.
     * @return string
     */
    public static function data_uri($text, $module_size = 5, $margin = 4)
    {
        if (!function_exists('imagecreatetruecolor') || !function_exists('imagepng') || !class_exists('WP_Arzo_QRCode')) {
            return '';
        }
        $qr  = WP_Arzo_QRCode::getMinimumQRCode((string) $text, WP_Arzo_QR_ERROR_CORRECT_LEVEL_M);
        $img = $qr->createImage((int) $module_size, (int) $margin);
        ob_start();
        imagepng($img);
        $png = ob_get_clean();
        imagedestroy($img);
    ...
    /** GD-free fallback: an HTML <table> QR for $text (larger DOM, but always works). */
    public static function html_table($text, $cell = '3px')
    {
        if (!class_exists('WP_Arzo_QRCode')) {
            return '';
        }
        $qr = WP_Arzo_QRCode::getMinimumQRCode((string) $text, WP_Arzo_QR_ERROR_CORRECT_LEVEL_M);
        ob_start();
...
define( 'WP_Arzo_QR_PAD0', 0xEC );
define( 'WP_Arzo_QR_PAD1', 0x11 );

class WP_Arzo_QRCode {
	...
	function __construct() {
		$this->typeNumber        = 1;
		$this->errorCorrectLevel = WP_Arzo_QR_ERROR_CORRECT_LEVEL_H;
	...
	function addData( $data, $mode = 0 ) {
		...
		if ( $mode == 0 ) {
			$mode = WP_Arzo_QRUtil::getMode( $data );
		}

		switch ( $mode ) {
			case WP_Arzo_QR_MODE_NUMBER:
				$impl = new WP_Arzo_QRNumber( $data );
				$this->addDataImpl( $impl );
				break;
			case WP_Arzo_QR_MODE_ALPHA_NUM:
				$impl = new WP_Arzo_QRAlphaNum( $data );
				$this->addDataImpl( $impl );
				break;
			case WP_Arzo_QR_MODE_8BIT_BYTE:
				$impl = new WP_Arzo_QR8BitByte( $data );
				$this->addDataImpl( $impl );
				break;
			case WP_Arzo_QR_MODE_KANJI:
				$impl = new WP_Arzo_QRKanji( $data );
				$this->addDataImpl( $impl );
				break;
	...
	function getBestMaskPattern() {
		...
		for ( $i = 0; $i < 8; $i ++ ) {
			...
			$lostPoint = WP_Arzo_QRUtil::getLostPoint( $this );
	...
	function makeImpl( $test, $maskPattern ) {
		...
		for ( $i = 0; $i < $this->moduleCount; $i ++ ) {
			$this->modules[] = WP_Arzo_QRCode::createNullArray( $this->moduleCount );
		}
		...
		$data = WP_Arzo_QRCode::createData( $this->typeNumber, $this->errorCorrectLevel, $dataArray );
			...
			while ( true ) {
				...
				for ( $c = 0; $c < 2; $c ++ ) {
					...
					if ( $this->modules[ $row ][ $col - $c ] === null ) {
						...
						if ( WP_Arzo_QRUtil::getMask( $maskPattern, $row, $col - $c ) ) {
							$dark = ! $dark;
						}
	...
	function setupPositionAdjustPattern() {
		...
		$pos = WP_Arzo_QRUtil::getPatternPosition( $this->typeNumber );
	...
	function setupTypeNumber( $test ) {
		...
		$bits = WP_Arzo_QRUtil::getBCHTypeNumber( $this->typeNumber );
	...
	function setupTypeInfo( $test, $maskPattern ) {
		...
		$bits = WP_Arzo_QRUtil::getBCHTypeInfo( $data );
	...
	function createData( $typeNumber, $errorCorrectLevel, $dataArray ) {
		...
		$rsBlocks = WP_Arzo_QRRSBlock::getRSBlocks( $typeNumber, $errorCorrectLevel );

		$buffer = new WP_Arzo_QRBitBuffer();

		for ( $i = 0; $i < count( $dataArray ); $i ++ ) {
			/** @var \WP_Arzo_QRData $data */
			$data = $dataArray[ $i ];
		...
		while ( true ) {
			...
			$buffer->put( WP_Arzo_QR_PAD0, 8 );

			if ( $buffer->getLengthInBits() >= $totalDataCount * 8 ) {
				break;
			}
			$buffer->put( WP_Arzo_QR_PAD1, 8 );
		}

		return WP_Arzo_QRCode::createBytes( $buffer, $rsBlocks );
	}

	/**
	 * @param \WP_Arzo_QRBitBuffer $buffer
	 * @param \WP_Arzo_QRRSBlock[] $rsBlocks
	 *
	 * @return array
	 */
	function createBytes( &$buffer, &$rsBlocks ) {
		...
		$dcdata = WP_Arzo_QRCode::createNullArray( count( $rsBlocks ) );
		$ecdata = WP_Arzo_QRCode::createNullArray( count( $rsBlocks ) );
		...
		for ( $r = 0; $r < $rsBlockCount; $r ++ ) {
			...
			$dcdata[ $r ] = WP_Arzo_QRCode::createNullArray( $dcCount );
			...
			$rsPoly  = WP_Arzo_QRUtil::getErrorCorrectPolynomial( $ecCount );
			$rawPoly = new WP_Arzo_QRPolynomial( $dcdata[ $r ], $rsPoly->getLength() - 1 );

			$modPoly      = $rawPoly->mod( $rsPoly );
			$ecdata[ $r ] = WP_Arzo_QRCode::createNullArray( $rsPoly->getLength() - 1 );
		...
		$data = WP_Arzo_QRCode::createNullArray( $totalCodeCount );
	...
	static function getMinimumQRCode( $data, $errorCorrectLevel ) {
		...
		for ( $typeNumber = 1; $typeNumber <= 40; $typeNumber ++ ) {
			if ( $length <= WP_Arzo_QRUtil::getMaxLength( $typeNumber, $mode, $errorCorrectLevel ) ) {
...
define( 'WP_Arzo_QR_G15', ( 1 << 10 ) | ( 1 << 8 ) | ( 1 << 5 )
                        | ( 1 << 4 ) | ( 1 << 2 ) | ( 1 << 1 ) | ( 1 << 0 ) );

define( 'WP_Arzo_QR_G18', ( 1 << 12 ) | ( 1 << 11 ) | ( 1 << 10 )
                        | ( 1 << 9 ) | ( 1 << 8 ) | ( 1 << 5 ) | ( 1 << 2 ) | ( 1 << 0 ) );

define( 'WP_Arzo_QR_G15_MASK', ( 1 << 14 ) | ( 1 << 12 ) | ( 1 << 10 )
                             | ( 1 << 4 ) | ( 1 << 1 ) );

class WP_Arzo_QRUtil {
	...
	static function getMaxLength( $typeNumber, $mode, $errorCorrectLevel ) {
		...
		switch ( $errorCorrectLevel ) {
			case WP_Arzo_QR_ERROR_CORRECT_LEVEL_L :
				$e = 0;
				break;
			case WP_Arzo_QR_ERROR_CORRECT_LEVEL_M :
				$e = 1;
				break;
			case WP_Arzo_QR_ERROR_CORRECT_LEVEL_Q :
				$e = 2;
				break;
			case WP_Arzo_QR_ERROR_CORRECT_LEVEL_H :
				$e = 3;
				break;
			default :
	...
	static function getErrorCorrectPolynomial( $errorCorrectLength ) {
		...
		$a = new WP_Arzo_QRPolynomial( array( 1 ) );

		for ( $i = 0; $i < $errorCorrectLength; $i ++ ) {
			$a = $a->multiply( new WP_Arzo_QRPolynomial( array( 1, WP_Arzo_QRMath::gexp( $i ) ) ) );
		}
	...
	static function getMask( $maskPattern, $i, $j ) {
		...
		switch ( $maskPattern ) {
			...
			case WP_Arzo_QR_MASK_PATTERN000 :
				return ( $i + $j ) % 2 == 0;
			case WP_Arzo_QR_MASK_PATTERN001 :
				return $i % 2 == 0;
			case WP_Arzo_QR_MASK_PATTERN010 :
				return $j % 3 == 0;
			case WP_Arzo_QR_MASK_PATTERN011 :
				return ( $i + $j ) % 3 == 0;
			case WP_Arzo_QR_MASK_PATTERN100 :
				return ( floor( $i / 2 ) + floor( $j / 3 ) ) % 2 == 0;
			case WP_Arzo_QR_MASK_PATTERN101 :
				return ( $i * $j ) % 2 + ( $i * $j ) % 3 == 0;
			case WP_Arzo_QR_MASK_PATTERN110 :
				return ( ( $i * $j ) % 2 + ( $i * $j ) % 3 ) % 2 == 0;
			case WP_Arzo_QR_MASK_PATTERN111 :
	...
	static function getMode( $s ) {
		if ( WP_Arzo_QRUtil::isAlphaNum( $s ) ) {
			if ( WP_Arzo_QRUtil::isNumber( $s ) ) {
				return WP_Arzo_QR_MODE_NUMBER;
			}

			return WP_Arzo_QR_MODE_ALPHA_NUM;
		} elseif ( WP_Arzo_QRUtil::isKanji( $s ) ) {
			return WP_Arzo_QR_MODE_KANJI;
		} else {
			return WP_Arzo_QR_MODE_8BIT_BYTE;
		}
	}
	...
	static function isNumber( $s ) {
		for ( $i = 0; $i < strlen( $s ); $i ++ ) {
			$c = ord( $s[ $i ] );
			if ( ! ( WP_Arzo_QRUtil::toCharCode( '0' ) <= $c && $c <= WP_Arzo_QRUtil::toCharCode( '9' ) ) ) {
				return false;
			}
		}
	...
	static function isAlphaNum( $s ) {
		for ( $i = 0; $i < strlen( $s ); $i ++ ) {
			$c = ord( $s[ $i ] );
			if ( ! ( WP_Arzo_QRUtil::toCharCode( '0' ) <= $c && $c <= WP_Arzo_QRUtil::toCharCode( '9' ) )
			     && ! ( WP_Arzo_QRUtil::toCharCode( 'A' ) <= $c && $c <= WP_Arzo_QRUtil::toCharCode( 'Z' ) )
			     && strpos( " $%*+-./:", $s[ $i ] ) === false ) {
				return false;
			}
		}
	...
	static function getBCHTypeInfo( $data ) {
		$d = $data << 10;
		while ( WP_Arzo_QRUtil::getBCHDigit( $d ) - WP_Arzo_QRUtil::getBCHDigit( WP_Arzo_QR_G15 ) >= 0 ) {
			$d ^= ( WP_Arzo_QR_G15 << ( WP_Arzo_QRUtil::getBCHDigit( $d ) - WP_Arzo_QRUtil::getBCHDigit( WP_Arzo_QR_G15 ) ) );
		}

		return ( ( $data << 10 ) | $d ) ^ WP_Arzo_QR_G15_MASK;
	}

	static function getBCHTypeNumber( $data ) {
		$d = $data << 12;
		while ( WP_Arzo_QRUtil::getBCHDigit( $d ) - WP_Arzo_QRUtil::getBCHDigit( WP_Arzo_QR_G18 ) >= 0 ) {
			$d ^= ( WP_Arzo_QR_G18 << ( WP_Arzo_QRUtil::getBCHDigit( $d ) - WP_Arzo_QRUtil::getBCHDigit( WP_Arzo_QR_G18 ) ) );
		}
...
class WP_Arzo_QRRSBlock {
	...
	static function getRSBlocks( $typeNumber, $errorCorrectLevel ) {
		...
		$rsBlock = WP_Arzo_QRRSBlock::getRsBlockTable( $typeNumber, $errorCorrectLevel );
		...
		for ( $i = 0; $i < $length; $i ++ ) {
			...
			for ( $j = 0; $j < $count; $j ++ ) {
				$list[] = new WP_Arzo_QRRSBlock( $totalCount, $dataCount );
			}
		}
...
class WP_Arzo_QRNumber extends WP_Arzo_QRData {

	function __construct( $data ) {
		parent::__construct( WP_Arzo_QR_MODE_NUMBER, $data );
	}
	...
	function write( &$buffer ) {
		...
		while ( $i + 2 < strlen( $data ) ) {
			$num = WP_Arzo_QRNumber::parseInt( substr( $data, $i, 3 ) );
			$buffer->put( $num, 10 );
			$i += 3;
		}
		...
		if ( $i < strlen( $data ) ) {
			...
			if ( strlen( $data ) - $i == 1 ) {
				$num = WP_Arzo_QRNumber::parseInt( substr( $data, $i, $i + 1 ) );
				$buffer->put( $num, 4 );
			} elseif ( strlen( $data ) - $i == 2 ) {
				$num = WP_Arzo_QRNumber::parseInt( substr( $data, $i, $i + 2 ) );
				$buffer->put( $num, 7 );
			}
		}
	}
	...
	static function parseInt( $s ) {
		...
		$num = 0;
		for ( $i = 0; $i < strlen( $s ); $i ++ ) {
			$num = $num * 10 + WP_Arzo_QRNumber::parseIntAt( ord( $s[ $i ] ) );
		}
	...
	static function parseIntAt( $c ) {
		...
		if ( WP_Arzo_QRUtil::toCharCode( '0' ) <= $c && $c <= WP_Arzo_QRUtil::toCharCode( '9' ) ) {
			return $c - WP_Arzo_QRUtil::toCharCode( '0' );
		}
...
class WP_Arzo_QRKanji extends WP_Arzo_QRData {

	function __construct( $data ) {
		parent::__construct( WP_Arzo_QR_MODE_KANJI, $data );
	}
...
class WP_Arzo_QRAlphaNum extends WP_Arzo_QRData {

	function __construct( $data ) {
		parent::__construct( WP_Arzo_QR_MODE_ALPHA_NUM, $data );
	}
	...
	function write( &$buffer ) {
		...
		while ( $i + 1 < strlen( $c ) ) {
			$buffer->put( WP_Arzo_QRAlphaNum::getCode( ord( $c[ $i ] ) ) * 45
			              + WP_Arzo_QRAlphaNum::getCode( ord( $c[ $i + 1 ] ) ), 11 );
			$i += 2;
		}

		if ( $i < strlen( $c ) ) {
			$buffer->put( WP_Arzo_QRAlphaNum::getCode( ord( $c[ $i ] ) ), 6 );
		}
	}

	static function getCode( $c ) {

		if ( WP_Arzo_QRUtil::toCharCode( '0' ) <= $c
		     && $c <= WP_Arzo_QRUtil::toCharCode( '9' ) ) {
			return $c - WP_Arzo_QRUtil::toCharCode( '0' );
		} elseif ( WP_Arzo_QRUtil::toCharCode( 'A' ) <= $c
		           && $c <= WP_Arzo_QRUtil::toCharCode( 'Z' ) ) {
			return $c - WP_Arzo_QRUtil::toCharCode( 'A' ) + 10;
		} else {
			switch ( $c ) {
				case WP_Arzo_QRUtil::toCharCode( ' ' ) :
					return 36;
				case WP_Arzo_QRUtil::toCharCode( '$' ) :
					return 37;
				case WP_Arzo_QRUtil::toCharCode( '%' ) :
...
class WP_Arzo_QR8BitByte extends WP_Arzo_QRData {

	function __construct( $data ) {
		parent::__construct( WP_Arzo_QR_MODE_8BIT_BYTE, $data );
	}
...
class WP_Arzo_QRMath {
	...
	static function init() {
		...
		self::$QR_MATH_EXP_TABLE = WP_Arzo_QRMath::createNumArray( 256 );
		...
		self::$QR_MATH_LOG_TABLE = WP_Arzo_QRMath::createNumArray( 256 );
...
class WP_Arzo_QRPolynomial {
	...
	function __construct( $num, $shift = 0 ) {
		...
		$this->num = WP_Arzo_QRMath::createNumArray( count( $num ) - $offset + $shift );
	...
	function toLogString() {
		...
		for ( $i = 0; $i < $this->getLength(); $i ++ ) {
			...
			$buffer .= WP_Arzo_QRMath::glog( $this->get( $i ) );
		}
	...
	/**
	 * @param \WP_Arzo_QRPolynomial $e
	 *
	 * @return \WP_Arzo_QRPolynomial
	 */
	function multiply( $e ) {

		$num = WP_Arzo_QRMath::createNumArray( $this->getLength() + $e->getLength() - 1 );

		for ( $i = 0; $i < $this->getLength(); $i ++ ) {
			$vi = WP_Arzo_QRMath::glog( $this->get( $i ) );

			for ( $j = 0; $j < $e->getLength(); $j ++ ) {
				$num[ $i + $j ] ^= WP_Arzo_QRMath::gexp( $vi + WP_Arzo_QRMath::glog( $e->get( $j ) ) );
			}
		}

		return new WP_Arzo_QRPolynomial( $num );
	}

	/**
	 * @param \WP_Arzo_QRPolynomial $e
	 *
	 * @return $this|\WP_Arzo_QRPolynomial
	 */
	function mod( $e ) {
		...
		$ratio = WP_Arzo_QRMath::glog( $this->get( 0 ) ) - WP_Arzo_QRMath::glog( $e->get( 0 ) );

		$num = WP_Arzo_QRMath::createNumArray( $this->getLength() );
		for ( $i = 0; $i < $this->getLength(); $i ++ ) {
			$num[ $i ] = $this->get( $i );
		}

		for ( $i = 0; $i < $e->getLength(); $i ++ ) {
			$num[ $i ] ^= WP_Arzo_QRMath::gexp( WP_Arzo_QRMath::glog( $e->get( $i ) ) + $ratio );
		}

		$newPolynomial = new WP_Arzo_QRPolynomial( $num );
...
define( 'WP_Arzo_QR_MODE_NUMBER', 1 << 0 );
define( 'WP_Arzo_QR_MODE_ALPHA_NUM', 1 << 1 );
define( 'WP_Arzo_QR_MODE_8BIT_BYTE', 1 << 2 );
define( 'WP_Arzo_QR_MODE_KANJI', 1 << 3 );

//---------------------------------------------------------------
// MaskPattern
//---------------------------------------------------------------

define( 'WP_Arzo_QR_MASK_PATTERN000', 0 );
define( 'WP_Arzo_QR_MASK_PATTERN001', 1 );
define( 'WP_Arzo_QR_MASK_PATTERN010', 2 );
define( 'WP_Arzo_QR_MASK_PATTERN011', 3 );
define( 'WP_Arzo_QR_MASK_PATTERN100', 4 );
define( 'WP_Arzo_QR_MASK_PATTERN101', 5 );
define( 'WP_Arzo_QR_MASK_PATTERN110', 6 );
define( 'WP_Arzo_QR_MASK_PATTERN111', 7 );

//---------------------------------------------------------------
// ErrorCorrectLevel

// 7%.
define( 'WP_Arzo_QR_ERROR_CORRECT_LEVEL_L', 1 );
// 15%.
define( 'WP_Arzo_QR_ERROR_CORRECT_LEVEL_M', 0 );
// 25%.
define( 'WP_Arzo_QR_ERROR_CORRECT_LEVEL_Q', 3 );
// 30%.
define( 'WP_Arzo_QR_ERROR_CORRECT_LEVEL_H', 2 );
