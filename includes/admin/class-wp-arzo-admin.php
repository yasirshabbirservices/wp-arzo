...
class WP_Arzo_Admin
{
    ...
    public function add_menu()
    {
        ...
        // Snippets is a Pro feature now (script insertion; not on wp.org) — the Pro add-on
        // registers its own "Snippets" submenu at PAGE_SNIPPETS. The free core keeps only the
        // shared editor assets (CodeMirror enqueue + snippetNonce) that the Pro page reuses.
    ...
    private function render_license_box()
    {
        ...
        <div class="wpa-aside-card wpa-license<?php echo esc_attr($card); ?>">
            ...
            <?php elseif ($pro) : ?>
                ...
                <?php endif; ?>
            <?php else : ?>
                <p class="wpa-aside-card__text"><?php echo $manage
                    ? 'Activate your WP Arzo Pro license to unlock every premium feature.'
                    : 'Unlock analytics, advanced security, cloud backups and more with WP Arzo Pro.'; ?></p>
                <div class="wpa-license__actions">
                    <?php if ($manage) : ?>
        ...
        <?php if (!$this->rendering_tab) : ?><div class="wrap wpa-admin"><?php endif; ?>
            ...
            <div class="wpa-card" style="margin-bottom:16px;">
                ...
                <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:flex-end;">
                    ...
                    <div class="wpa-field" style="min-width:150px;margin:0;">
                        <label class="wpa-field__label" for="wpa-rest-scope">Access</label>
            ...
            <div class="wpa-card" style="padding:0;overflow:hidden;">
                <table class="wpa-backup-table" id="wpa-rest-table">
                    <thead><tr><th>Label</th><th>Key</th><th>Access</th><th>Authenticates as</th><th>Created (UTC)</th><th>Last used (UTC)</th><th>Expires (UTC)</th><th></th></tr></thead>
                    <tbody>
                        <?php if (empty($keys)) : ?>
                            <tr class="wpa-backup-empty"><td colspan="8">No keys yet. Generate one above.</td></tr>
...
curl -u "any:arzo_…" <?php echo esc_html($example); ?></code></pre>
    ...
    public function ajax_rest_key_create()
    {
        ...
        wp_send_json_success(array(
            ...
            'scope' => isset($res['scope']) ? $res['scope'] : 'full',
        ));
    }
