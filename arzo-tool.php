                    ...
                    <div class="info-item">
                        ...
                        <span class="info-value">
                            <div class="disk-usage">
                                ...
                                <div class="progress-bar">
                                    <div class="progress-fill" style="width: <?php echo $total_plugins > 0 ? ($active_plugin_count / $total_plugins) * 100 : 0; ?>%;"></div>
                                </div>
                            </div>
                        </span>
                    </div>
            ...
            <div class="info-card">
                ...
                <div class="info-card-body">
                    <div class="info-item">
                        ...
                        <span class="info-value">
                            <span class="status-badge <?php echo defined('DISALLOW_FILE_EDIT') && DISALLOW_FILE_EDIT ? 'status-enabled' : 'status-warning'; ?>">
                    ...
                    <div class="info-item">
                        ...
                        <span class="info-value">
                            <span class="status-badge <?php echo defined('DISABLE_WP_CRON') && DISABLE_WP_CRON ? 'status-disabled' : 'status-enabled'; ?>">
                    ...
                    <div class="info-item">
                        ...
                        <span class="info-value">
                            <span class="status-badge <?php echo defined('AUTOMATIC_UPDATER_DISABLED') && AUTOMATIC_UPDATER_DISABLED ? 'status-disabled' : 'status-enabled'; ?>">
                    ...
                    <div class="info-item">
                        ...
                        <span class="info-value">
                            <span class="status-badge <?php echo defined('SCRIPT_DEBUG') && SCRIPT_DEBUG ? 'status-warning' : 'status-disabled'; ?>">
                    ...
                    <div class="info-item">
                        ...
                        <span class="info-value">
                            <span class="status-badge <?php echo defined('WP_CACHE') && WP_CACHE ? 'status-enabled' : 'status-disabled'; ?>">
...
function handleUsers() {
    ...
    <div class="content">
        ...
        <div style="background: #2A2A2A; padding: 15px; border-radius: 3px; border-left: 4px solid var(--accent-color); margin-bottom: 20px;">
            <?php
        ...
        <table>
            ...
            foreach ($users as $user) {
                ...
                $row_class = $is_current ? 'style="background-color: rgba(22, 231, 145, 0.1);"' : '';
                
                echo '<tr ' . $row_class . '>';
                echo '<td>' . $user->ID . '</td>';
                echo '<td>' . $user->user_login . ($is_current ? ' <i class="fas fa-user" style="color: var(--accent-color);" title="Current User"></i>' : '') . '</td>';
                echo '<td>' . $user->user_email . '</td>';
                echo '<td>' . implode(', ', $user->roles) . '</td>';
                echo '<td>' . ($is_current ? '<span style="color: var(--accent-color); font-weight: bold;">Logged In</span>' : '<span style="color: #999;">Offline</span>') . '</td>';
                ...
                if (!$is_current) {
                    ...
                    echo '<button type="submit" name="quick_login" class="btn btn-success" title="Login as ' . $user->user_login . '" style="background: var(--success-color); border: none; padding: 5px 10px; font-size: 12px;"><i class="fas fa-sign-in-alt"></i> Login</button>';
                    echo '</form>';
                } else {
                    echo '<span style="color: var(--accent-color); font-size: 12px; margin-right: 5px;"><i class="fas fa-check-circle"></i> Active</span>';
                }
                
                // Delete Button (don't allow deleting current user)
                if (!$is_current) {
                    echo '<form method="post" style="display:inline;">';
                    echo '<input type="hidden" name="user_id" value="' . $user->ID . '">';
                    echo '<button type="submit" name="delete_user" class="btn btn-danger" onclick="return confirm(\'Are you sure you want to delete user: ' . $user->user_login . '?\');" title="Delete ' . $user->user_login . '" style="background: var(--danger-color); border: none; padding: 5px 10px; font-size: 12px;"><i class="fas fa-trash"></i> Delete</button>';
                    echo '</form>';
                } else {
                    echo '<span style="color: #666; font-size: 12px;">Cannot delete current user</span>';
                }
    ...
    <div class="content">
        ...
        <?php if ($config_writable): ?>
            <form method="post" style="background: #2A2A2A; padding: 20px; border-radius: 3px; border: 1px solid #333333;">
                ...
                <div class="debug-settings-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 15px 15px; margin-bottom: 20px;">
                    <?php foreach ($debug_settings as $setting => $info): ?>
                        <div class="form-group" style="display: flex; align-items: center; justify-content: space-between; padding: 12px 15px; background: #1a1a1a; border-radius: 5px; border: 1px solid #333;">
                            <div style="flex: 1; min-width: 0;">
                                <label style="display: block; margin-bottom: 3px; font-weight: bold; color: #fff; font-size: 14px;"><?php echo $setting; ?></label>
                                <p style="font-size: 11px; color: #999; margin: 0; line-height: 1.3; overflow: hidden; text-overflow: ellipsis;"><?php echo $info['description']; ?></p>
                            </div>
                            <div style="margin-left: 15px; flex-shrink: 0;">
                                <label class="switch">
                                    <input type="checkbox" name="<?php echo $setting; ?>" value="true" <?php echo ($info['current'] === 'true') ? 'checked' : ''; ?> onchange="logDebugChange('<?php echo $setting; ?>', this.checked)">
