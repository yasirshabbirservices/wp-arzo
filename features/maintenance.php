...
if (isset($_GET['operation']) && $_GET['operation'] === 'update_maintenance_option') {
    ...
    if (isset($_POST['option_name']) && isset($_POST['option_value'])) {
        ...
        } elseif ($option === 'maintenance_tool_custom_css') {
            $value = wp_strip_all_tags($_POST['option_value']);
        } else {
...
@keyframes pulse {
    0%, 100% { box-shadow: 0 0 0 0 rgba(22, 231, 145, 0.7); }
    50% { box-shadow: 0 0 0 8px rgba(22, 231, 145, 0); }
}
@keyframes blink {
    0%, 100% { opacity: 1; }
    50% { opacity: 0.3; }
...
.mode-maintenance .mode-icon { color: #ff9800; }
.mode-coming-soon .mode-icon { color: #4CAF50; }
.mode-payment .mode-icon { color: #dc3545; }
...
.mode-maintenance .btn-activate { background: linear-gradient(to right, #ff9800, #f57c00); }
.mode-maintenance .btn-activate:hover { box-shadow: 0 4px 12px rgba(255, 152, 0, 0.3); }

.mode-coming-soon .btn-activate { background: linear-gradient(to right, #4CAF50, #388E3C); }
.mode-coming-soon .btn-activate:hover { box-shadow: 0 4px 12px rgba(76, 175, 80, 0.3); }

.mode-payment .btn-activate { background: linear-gradient(to right, #dc3545, #c82333); }
.mode-payment .btn-activate:hover { box-shadow: 0 4px 12px rgba(220, 53, 69, 0.3); }
...
.mode-card.active .btn-deactivate { display: flex; }
.mode-card.active .btn-activate { display: none; }
...
<div class="content maintenance-container">
    ...
    <div class="settings-box">
        <h3 style="margin-top: 0; margin-bottom: 20px; font-size: 16px;"><i class="fas fa-address-book"></i> Social Contact Settings</h3>
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 20px;">
            <div class="form-group">
                <label>Email Address</label>
                <input type="email" class="form-control auto-save" data-option="maintenance_tool_developer_email" value="<?php echo esc_attr($developer_email); ?>">
            </div>
            <div class="form-group">
                <label>Phone Number</label>
                <input type="text" class="form-control auto-save" data-option="maintenance_tool_developer_phone" value="<?php echo esc_attr($developer_phone); ?>" placeholder="+1234567890">
            </div>
            <div class="form-group">
                <label>WhatsApp Number</label>
                <input type="text" class="form-control auto-save" data-option="maintenance_tool_developer_whatsapp" value="<?php echo esc_attr($developer_whatsapp); ?>" placeholder="+1234567890">
            </div>
            <div class="form-group">
                <label>Skype Username</label>
                <input type="text" class="form-control auto-save" data-option="maintenance_tool_developer_skype" value="<?php echo esc_attr($developer_skype); ?>">
            </div>
        </div>
    </div>
    ...
    <div class="mode-grid">
        ...
        <!-- Maintenance Mode -->
        <div class="mode-card mode-maintenance <?php echo $current_mode === 'maintenance' ? 'active' : ''; ?>" id="card-maintenance">
            ...
            <div class="info-alert">
                <svg viewBox="0 0 20 20"><path d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z"/></svg>
                <span class="info-alert-text">Your site is currently in maintenance mode.</span>
            </div>

            <div class="form-group">
                <label>Page Title</label>
                <input type="text" class="form-control auto-save" data-option="maintenance_tool_custom_title" value="<?php echo $current_mode === 'maintenance' ? esc_attr($custom_title) : 'Site Under Maintenance'; ?>">
            </div>

            <div class="form-group">
                <label>Message</label>
                <textarea class="form-control auto-save" rows="3" data-option="maintenance_tool_custom_message"><?php echo $current_mode === 'maintenance' ? esc_textarea($custom_message) : 'We are currently performing scheduled maintenance. Please check back soon.'; ?></textarea>
            </div>

            <div class="switch-wrapper">
                <label class="switch">
                    <input type="checkbox" onchange="toggleOption('maintenance_tool_show_social_contacts', this.checked)" <?php echo $show_social_contacts ? 'checked' : ''; ?>>
        ...
        <!-- Coming Soon Mode -->
        <div class="mode-card mode-coming-soon <?php echo $current_mode === 'coming_soon' ? 'active' : ''; ?>" id="card-coming_soon">
            <div class="status-badge">ACTIVE</div>
            ...
            <div class="form-group">
                <label>Page Title</label>
                <input type="text" class="form-control auto-save" data-option="maintenance_tool_custom_title" value="<?php echo $current_mode === 'coming_soon' ? esc_attr($custom_title) : 'Coming Soon'; ?>">
            </div>

            <div class="form-group">
                <label>Message</label>
                <textarea class="form-control auto-save" rows="3" data-option="maintenance_tool_custom_message"><?php echo $current_mode === 'coming_soon' ? esc_textarea($custom_message) : 'Something amazing is coming soon! Stay tuned for updates.'; ?></textarea>
            </div>
        ...
        <div class="mode-card mode-payment <?php echo $current_mode === 'payment_request' ? 'active' : ''; ?>" id="card-payment_request">
            <div class="status-badge">ACTIVE</div>
            ...
            <div class="form-group">
                <label>Page Title</label>
                <input type="text" class="form-control auto-save" data-option="maintenance_tool_custom_title" value="<?php echo $current_mode === 'payment_request' ? esc_attr($custom_title) : 'Payment Required'; ?>">
            </div>

            <div class="form-group">
                <label>Message</label>
                <textarea class="form-control auto-save" rows="3" data-option="maintenance_tool_custom_message"><?php echo $current_mode === 'payment_request' ? esc_textarea($custom_message) : 'This website has been completed but payment is still pending.'; ?></textarea>
            </div>
    ...
    <!-- Custom CSS -->
    <div class="settings-box">
        <h3 style="margin-top: 0; margin-bottom: 20px; font-size: 16px;">Custom CSS</h3>
        <textarea class="form-control auto-save" rows="6" data-option="maintenance_tool_custom_css" style="font-family: monospace; font-size: 12px; height: 150px;" placeholder="/* Add custom CSS here */"><?php echo esc_textarea($custom_css); ?></textarea>
    </div>
...
function updateUIState() {
    ...
    document.querySelectorAll('.mode-card').forEach(card => {
        ...
        // Reset inputs to default values if needed? No, keep them as is (user choice).
    });
